...
Policy gradient methods is similar to supervised learning in that when we have collected the whole episode, calculated the future discounted reward, we have created a setup that is similar to supervised learning of X, Y target pairs (say training a convolutional neural network to regonize images).
